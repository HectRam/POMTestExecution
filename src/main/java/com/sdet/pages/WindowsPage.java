	...
	public void aboutWindowsMenulValidateAllItems() {
		...
		if (ListItems.size() > 0 && ListItems.size() <= 12) {
			for (WebElement e : ListItems) {
				...
				ExtentListeners.getExtent().log(Status.INFO, "List Item Text : : " + e.getText());
			}
			ExtentListeners.getExtent().log(Status.PASS, "About Windows Items were listed successfuly");
