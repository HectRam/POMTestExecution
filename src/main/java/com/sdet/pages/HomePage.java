...
public class HomePage extends MainPage {
	...
	public WindowsPage goToWindows() {
		...
		Click("Windows_X");
		return new WindowsPage(getDriver());
	}
		...
		if (ListItems.size() <= 9) {
			...
			for (WebElement e : ListItems) {
				...
				if (e.getText().equals(names.get(n))) {
					...
					ExtentListeners.getExtent().log(Status.PASS, e.getText() + " is Present");
					log.info(e.getText() + " is Present");
				} else {
					ExtentListeners.getExtent().log(Status.FAIL, e.getText() + " is Absent");
	...
	public SoftwareVisualStudioPage searchFor(String Value) {
		...
		waitForvisibilityOf("waitForVSSearch_X");
		...
		for (int i = 0; i < LIST.size(); i++) {
			...
			if (LIST.get(i).getText().contains(Value)) {
				...
				ExtentListeners.getExtent().log(Status.PASS, Value + " Found and Clicked");
			} else {
				ExtentListeners.getExtent().log(Status.FAIL, Value + " was not Found");
				log.error(Value + " was not Found");
				Assert.fail(Value + " was not Found");
			}
		}
		return new SoftwareVisualStudioPage(getDriver());
	}
